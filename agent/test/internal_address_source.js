...
describe('configmap backed address source', function() {
    ...
    it('retrieves all addresses', function(done) {
        ...
        var source = new AddressSource({name:'foo',uuid: '1234'}, {port:configmaps.port, host:'localhost', token:'foo', namespace:'default'});
    ...
    it('retrieves address types from plans', function(done) {
        ...
        var source = new AddressSource({name:'foo',plan:'space'}, {port:configmaps.port, host:'localhost', token:'foo', namespace:'default'});
        source.watcher.close();
    ...
    it('creates an address', function(done) {
        var source = new AddressSource({namespace: 'ns1', name: 'foo', uuid: '1234'}, {port:configmaps.port, host:'localhost', token:'foo', namespace:'default'});
        source.once('addresses_defined', function () {
            source.create_address({address:'myqueue', type:'queue', plan:'clever'}).then(
                function () {
                    source.once('addresses_defined', function (addresses) {
                        assert.equal(addresses.length, 1);
                        assert.equal(addresses[0].name.substring(0,8), 'ns1.foo.');
    ...
    it('deletes an address', function(done) {
        configmaps.add_address_definition({address:'foo', type:'queue'}, 'address-config-foo', '1234');
        configmaps.add_address_definition({address:'bar', type:'topic'}, 'address-config-bar', '1234');
        var source = new AddressSource({name: 'foo', uuid: '1234'}, {port:configmaps.port, host:'localhost', token:'foo', namespace:'default'});
    ...
    it('retrieves plans concurrently with addresses', function(done) {
        ...
        configmaps.add_address_plan({plan_name:'bar', address_type:'topic'});
        configmaps.add_address_plan({plan_name:'standard', address_type:'anycast', display_name:'display me', shortDescription:'abcdefg', longDescription:'hijklmn'});
        var source = new AddressSource({name: 'foo', plan: 'space', uuid: '1234'}, {port:configmaps.port, host:'localhost', token:'foo', namespace:'default'});
